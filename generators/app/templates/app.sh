...
if [ "$#" -eq 0 ] || [ $1 = "-h" ] || [ $1 = "--help" ]; then
    ...
    echo ""
    echo "Commands:"
    echo "  $CLEAN      - Stop and Remove image containers."
    echo "  $RUN        - Build and Run image."
...
stop_existing() {
  APP="$(docker ps --all --quiet --filter=name=$AppName)"
  REDIS="$(docker ps --all --quiet --filter=name=$AppName-redis)"
  MONGO="$(docker ps --all --quiet --filter=name=$AppName-mongo)"
