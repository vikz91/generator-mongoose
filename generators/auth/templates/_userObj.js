...
api.getAllUsers = function(skip, limit, role, status, cb) {
    ...
    var q = User.find(
        sObj,
        '_id  email profile role status projects address phone'
    );
...
api.getUser = function(id, cb) {
    User.findOne({ _id: id })
        .populate(
            'projects',
            '_id name status locked startdate deadline designer customer'
        )
        .exec((err, user) => {
            if (user === null) {
...
api.uploadAvatar = function(req, res, next) {
    ...
    fs.rename(req.file.path, fileUri, function(err) {
        ...
        return res.status(200).json({
            url: req.headers.host + '/' + l.DIR_AVATAR + filename,
            path: fileUri
        });
    });
};
...
api.editUser = function(id, updateData, cb) {
    if (updateData === undefined) {
        return cb(
            'Invalid Data. Please Check user and/or updateData fields',
            null
        );
    }
